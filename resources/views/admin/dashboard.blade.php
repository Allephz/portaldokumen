...
@section('title', 'Admin Dashboard - Portal ISO 9001')

@section('content')
<div class="page-header">
    <h2><i class="bi bi-shield-lock"></i> Admin Dashboard <span id="adminCategoryTitle"></span></h2>
    <p>Kelola semua aspek sistem Portal ISO 9001</p>
</div>
...
function uploadFileAsAdmin() {
    ...
    let categoryOptions = `Silakan pilih kategori:\n\n`;
    let categoryId = null;
    
    const categories = [
        { id: 1, name: 'Folder Manual' },
        { id: 2, name: 'SOP' },
        { id: 3, name: 'Instruksi Kerja' },
        { id: 4, name: 'Catatan/Rekaman' }
    ];
    
    let categoryList = categories.map((cat, idx) => `${idx + 1}. ${cat.name}`).join('\n');
    let choice = prompt(categoryList + `\n\nMasukkan nomor kategori (1-4):`, '1');
    ...
    categoryId = categories[choiceNum - 1].id;
